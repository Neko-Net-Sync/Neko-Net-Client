...
namespace NekoNetClient.Interop.Ipc;
...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(ILogger logger, string uid)
    {
        ...
        return await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            _penumbraCreateNamedTemporaryCollection.Invoke("Nekosync", collName, out Guid collId); logger.LogTrace("Creating Temp Collection {collName}, GUID: {collId}", collName, collId);
            return collId;
    ...
    {
        if (!APIAvailable) return;
        // Avoid hitting Penumbra during global draw/redraw; serialize cleanups
        await _cleanupSemaphore.WaitAsync().ConfigureAwait(false);
        try
        {
            var start = DateTime.UtcNow;
            while ((_dalamudUtil.IsAnythingDrawing || _dalamudUtil.IsZoning) && DateTime.UtcNow - start < TimeSpan.FromSeconds(3))
                await Task.Delay(50).ConfigureAwait(false);

            await _dalamudUtil.RunOnFrameworkThread(() =>
            {
                logger.LogTrace("[{applicationId}] [FW-ENTER] Removing temp collection for {collId}", applicationId, collId);
                var ret2 = _penumbraRemoveTemporaryCollection.Invoke(collId);
                logger.LogTrace("[{applicationId}] [FW-EXIT] RemoveTemporaryCollection: {ret2}", applicationId, ret2);
            }).ConfigureAwait(false);
        }
        finally
        {
