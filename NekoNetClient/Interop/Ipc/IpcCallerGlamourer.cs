...
public sealed class IpcCallerGlamourer : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    {
        if ((!APIAvailable) || _dalamudUtil.IsZoning) return;
        // Avoid reverting during global draw/redraw to reduce risk of stalls
        var start = DateTime.UtcNow;
        while ((_dalamudUtil.IsAnythingDrawing || _dalamudUtil.IsZoning) && DateTime.UtcNow - start < TimeSpan.FromSeconds(3))
            await Task.Delay(50).ConfigureAwait(false);

        await _dalamudUtil.RunOnFrameworkThread(() => RevertByName(logger, name, applicationId)).ConfigureAwait(false);
    }
