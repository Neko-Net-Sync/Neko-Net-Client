...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    public SettingsUi(ILogger<SettingsUi> logger,
        ...
        DalamudUtilService dalamudUtilService, HttpClient httpClient, MultiHubManager multiHub) : base(logger, mediator, "Neko-Net Settings", performanceCollector)
    {
        ...
        SizeConstraints = new WindowSizeConstraints()
        {
            ...
            // Allow wider layouts for tables and forms
    ...
    // ---- Server Profiles UI ----
    // removed legacy server-profiles editor state
    // removed legacy server-profiles editor
    ...
    private void DrawServerConfiguration()
    {
        ...
        // Server quick-add/quick-connect removed; use 'Additional Servers' tab for server management
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                // API Endpoint
                var apiEndpoint = selectedServer.ApiEndpoint ?? "/mare";
                if (ImGui.InputText("API Endpoint", ref apiEndpoint, 255, flags))
                {
                    if (string.IsNullOrWhiteSpace(apiEndpoint)) apiEndpoint = "/mare";
                    if (!apiEndpoint.StartsWith('/')) apiEndpoint = "/" + apiEndpoint;
                    selectedServer.ApiEndpoint = apiEndpoint;
                    _serverConfigurationManager.Save();
                }
                _uiShared.DrawHelpText("Relative path to the hub endpoint. Examples: /mare, /lightless, /tera-sync-v2");
    ...
    private bool _addNekoNet = false, _addLightless = false, _addTera = false, _addXivSync = false, _addPlayerSync = false, _addAnatoli = false;
    private string _hostNekoNet = "connect.neko-net.cc";
    private string _hostLightless = "sync.lightless-sync.org";
    private string _hostTera = "tera.terasync.app";
    private string _hostXivSync = string.Empty;
    private string _hostPlayerSync = string.Empty;
    private string _hostAnatoli = string.Empty;

    // removed legacy quick-add helpers
    ...
    private void DrawSettingsContent()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Neko-Net Discord"))
        {
        ...
        if (ImGui.BeginTabBar("mainTabBar"))
        {
            ...
            if (ImGui.BeginTabItem("Additional Servers"))
            {
                DrawAdditionalServers();
                ImGui.EndTabItem();
            }
    ...
    private string _addSrvName = string.Empty;
    private string _addSrvUrl = string.Empty;
    private string _addSrvEndpoint = string.Empty;
    private int _editSrvIndex = -1;
    private string _editSrvName = string.Empty;
    private string _editSrvUrl = string.Empty;
    private string _editSrvEndpoint = string.Empty;

    private void DrawAdditionalServers()
    {
        _lastTab = "Additional Servers";

        using (_uiShared.UidFont.Push()) ImGui.TextUnformatted("Additional Servers");
        ImGuiHelpers.ScaledDummy(new Vector2(4, 4));
        ImGui.TextWrapped("Manage extra servers here. Add by URL; endpoint defaults by host. Use the Services window to connect to any configured server.");
        ImGui.Separator();

        // Add server form
        ImGui.TextUnformatted("Add Server");
        ImGuiHelpers.ScaledDummy(new Vector2(2,2));
        ImGui.SetNextItemWidth(240); ImGui.InputText("Name", ref _addSrvName, 100);
        ImGui.SameLine();
        ImGui.SetNextItemWidth(MathF.Max(300, UiSharedService.GetWindowContentRegionWidth() - 560));
        ImGui.InputText("URL (wss://host)", ref _addSrvUrl, 256);
        ImGui.SameLine();
        ImGui.SetNextItemWidth(200); ImGui.InputText("Endpoint (optional)", ref _addSrvEndpoint, 64);
        // Place the Add button on the next line so it doesn't get clipped on narrow windows
        ImGuiHelpers.ScaledDummy(new Vector2(2, 2));
        using (ImRaii.Disabled(string.IsNullOrWhiteSpace(_addSrvName) || string.IsNullOrWhiteSpace(_addSrvUrl)))
        {
            if (ImGui.Button("Add"))
            {
                var url = _addSrvUrl.Trim();
                if (!url.StartsWith("ws", StringComparison.OrdinalIgnoreCase) && !url.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                    url = "wss://" + url;
                var ep = string.IsNullOrWhiteSpace(_addSrvEndpoint) ? _serverConfigurationManager.GetApiEndpointForDomain(url) : _addSrvEndpoint.Trim();
                _serverConfigurationManager.AddServer(new ServerStorage
                {
                    ServerName = _addSrvName.Trim(),
                    ServerUri = url,
                    ApiEndpoint = ep,
                    UseOAuth2 = true,
                });
                _addSrvName = string.Empty; _addSrvUrl = string.Empty; _addSrvEndpoint = string.Empty;
            }
        }
        ...
        if (ImGui.BeginTable("srv_tbl", 5, ImGuiTableFlags.SizingStretchProp | ImGuiTableFlags.RowBg))
        {
            ...
            for (int i = 0; i < servers.Count; i++)
            {
                ...
                if (_editSrvIndex == i)
                {
                    ...
                    ImGui.InputText("##name", ref _editSrvName, 100);
                }
                else
                {
                    ImGui.TextUnformatted(string.IsNullOrWhiteSpace(s.ServerName) ? "(unnamed)" : s.ServerName);
                }
                ...
                {
                    ImGui.SetNextItemWidth(-1);
                    ImGui.InputText("##url", ref _editSrvUrl, 256);
                }
                else
                {
                    ImGui.TextUnformatted(s.ServerUri);
                }
                ...
                {
                    ImGui.SetNextItemWidth(-1);
                    ImGui.InputText("##ep", ref _editSrvEndpoint, 64);
                }
                else
                {
                ...
                {
                    var isCurrent = _serverConfigurationManager.CurrentServerIndex == i;
                    if (_editSrvIndex == i)
                    {
                        if (ImGui.SmallButton("Save"))
                        {
                            var newUrl = _editSrvUrl?.Trim() ?? string.Empty;
                            if (!newUrl.StartsWith("ws", StringComparison.OrdinalIgnoreCase) && !newUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                                newUrl = "wss://" + newUrl;
                            var newEp = string.IsNullOrWhiteSpace(_editSrvEndpoint) ? _serverConfigurationManager.GetApiEndpointForDomain(newUrl) : _editSrvEndpoint.Trim();

                            s.ServerName = _editSrvName?.Trim() ?? string.Empty;
                            s.ServerUri = newUrl;
                            s.ApiEndpoint = newEp;
                            _serverConfigurationManager.Save();
                            _editSrvIndex = -1;
                    ...
                    {
                        using (ImRaii.Disabled(isCurrent))
                        {
                            if (ImGui.SmallButton(isCurrent ? "Current" : "Use"))
                            {
                        ...
                        {
                            _editSrvIndex = i;
                            _editSrvName = s.ServerName ?? string.Empty;
                            _editSrvUrl = s.ServerUri ?? string.Empty;
                            _editSrvEndpoint = s.ApiEndpoint ?? _serverConfigurationManager.GetApiEndpointForDomain(s.ServerUri);
                        }
                        ImGui.SameLine();
                        using (ImRaii.Disabled(i == 0))
                        {
                            ...
                            {
                                _serverConfigurationManager.DeleteServer(s);
                                if (_editSrvIndex == i) _editSrvIndex = -1;
                            }
                        }
                    }
                }
