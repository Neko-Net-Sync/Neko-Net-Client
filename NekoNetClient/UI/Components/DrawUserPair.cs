...
{
    protected readonly IApiActionRouter _api;
    ...
    public DrawUserPair(string id, Pair entry, List<GroupFullInfoDto> syncedGroups,
        ...
        IApiActionRouter apiController, IdDisplayHandler uIDDisplayHandler,
    ...
    private void DrawCommonClientMenu()
    {
        ...
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.PlayCircle, "Cycle pause state", _menuWidth, true))
        {
            ...
            permissions.SetPaused(!permissions.IsPaused());
            _ = _api.UserSetPairPermissions(new UserPermissionsDto(_pair.UserData, permissions));
            ImGui.CloseCurrentPopup();
        }
        ImGui.Separator();
        ...
        if (_uiSharedService.IconTextButton(stickyIcon, stickyText, _menuWidth, true))
        {
            ...
            _ = _api.UserSetPairPermissions(new(_pair.UserData, permissions));
        }
        ...
        bool individual = !_pair.IsDirectlyPaired && _api.DefaultPermissions!.IndividualIsSticky;
    ...
    private void DrawIndividualMenu()
    {
        ...
        if (_pair.IndividualPairStatus != IndividualPairStatus.None)
        {
            ...
            {
                _ = _api.UserRemovePair(new(_pair.UserData));
            }
        ...
        else
        {
            ...
            {
                _ = _api.UserAddPair(new(_pair.UserData));
            }
    ...
    private void DrawPairedClientMenu()
    {
        ...
        {
            bool selfIsOwner = string.Equals(_api.UID, entry.Owner.UID, StringComparison.Ordinal);
    ...
    private float DrawRightSide()
    {
        ...
        if (_uiSharedService.IconButton(pauseIcon))
        {
            ...
            _ = _api.UserSetPairPermissions(new(_pair.UserData, perm));
        }
        if (_pair.IsApplying)
            ImGui.EndDisabled();
        UiSharedService.AttachToolTip(_pair.IsApplying
            ? "Busy applying character data; pausing will be deferred briefly"
    ...
    private void DrawSyncshellMenu(GroupFullInfoDto group, bool selfIsOwner, bool selfIsModerator, bool userIsPinned, bool userIsModerator)
    {
        if (selfIsOwner || ((selfIsModerator) && (!userIsModerator)))
        {
            ...
            if (_uiSharedService.IconTextButton(FontAwesomeIcon.Thumbtack, pinText, _menuWidth, true))
            {
                ...
                _ = _api.GroupSetUserInfo(new GroupPairUserInfoDto(group.Group, _pair.UserData, userinfo));
            }
            ...
            {
                ImGui.CloseCurrentPopup();
                _ = _api.GroupRemoveUser(new(group.Group, _pair.UserData));
            }
        ...
        if (selfIsOwner)
        {
            ...
            {
                ImGui.CloseCurrentPopup();
                _ = _api.GroupChangeOwnership(new(group.Group, _pair.UserData));
            }
