...
public abstract class DrawFolderBase : IDrawFolder
{
    ...
    private float DrawRightSideInternal()
    {
        ...
        if (RenderMenu)
        {
            ...
            {
                // Use a unique popup id per folder to prevent clashes and odd z-ordering
                ImGui.OpenPopup($"User Flyout Menu##{_id}");
            }
            // Ensure popup appears on top and near the trigger
            ImGui.SetNextWindowPos(ImGui.GetMousePos(), ImGuiCond.Appearing, System.Numerics.Vector2.Zero);
            ImGui.SetNextWindowFocus();
            if (ImGui.BeginPopup($"User Flyout Menu##{_id}"))
            {
