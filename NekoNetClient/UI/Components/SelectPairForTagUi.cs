...
public class SelectPairForTagUi
{
    ...
    private TagHandler? _activeTagHandlerOverride;
    ...
    public void Draw(List<Pair> pairs)
    {
        ...
        if (ImGui.BeginPopupModal(popupName, ref _show, ImGuiWindowFlags.Popup | ImGuiWindowFlags.Modal))
        {
                ...
                .ToList())
            {
                ...
                {
                    var th = _activeTagHandlerOverride ?? _tagHandler;
                    if (isInGroup)
                    {
                        th.AddTagToPairedUid(item.UserData.UID, _tag);
                        _peopleInGroup.Add(item.UserData.UID);
                    }
                    else
                    {
                        th.RemoveTagFromPairedUid(item.UserData.UID, _tag);
        ...
        else
        {
            ...
            _show = false;
            _activeTagHandlerOverride = null;
        }
    }
    ...
    // Overload to open with a specific TagHandler (service-scoped)
    public void Open(string tag, TagHandler tagHandler)
    {
        _activeTagHandlerOverride = tagHandler;
        _peopleInGroup = tagHandler.GetOtherUidsForTag(tag);
        _tag = tag;
        _show = true;
    }
