...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        ServerConfigurationManager serverManager, TokenProvider tokenProvider, MareMediator mediator,
        ISyncFacade syncFacade) : base(logger, mediator)
    {
    ...
    public void DrawCacheDirectorySetting()
    {
        ...
        using (ImRaii.Disabled(_cacheMonitor.MareWatcher != null))
        {
            ...
            {
                FileDialogManager.OpenFolderDialog("Pick Neko-Net Storage Folder", (success, path) =>
                {
                    if (!success) return;
                ...
                using (ImRaii.Disabled(!CtrlPressed()))
                {
                    if (IconTextButton(FontAwesomeIcon.Exclamation, "Renew OAuth2 token manually") && CtrlPressed())
                    {
                        _ = _tokenProvider.TryUpdateOAuth2LoginTokenAsync(selectedServer, forced: true)
                            .ContinueWith((_) => _syncFacade.StartAsync(CancellationToken.None));
                    }
                }
                ...
                if (IconTextButton(FontAwesomeIcon.Exclamation, "Renew OAuth2 connection"))
                {
                    ...
                    _ = _serverConfigurationManager.CheckDiscordOAuth(selectedServer.ServerUri)
                        .ContinueWith(async (urlTask) =>
                        {
                            ...
                            _serverConfigurationManager.Save();
                            await _syncFacade.StartAsync(CancellationToken.None).ConfigureAwait(false);
                        });
                }
            }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use Neko-Net.");
            return false;
        }
