...
public sealed class TokenProvider : IDisposable, IMediatorSubscriber
{
        ...
        catch (HttpRequestException ex)
        {
            ...
            if (ex.StatusCode == System.Net.HttpStatusCode.Unauthorized)
            {
                if (isRenewal)
                    ...
                    Mediator.Publish(new NotificationMessage("Error generating token", "Your authentication token could not be generated. Check Mares Main UI (/nsync in chat) to see the error message.",
                    NotificationType.Error));
    ...
    // --- Per-server variants (for multi-service hubs) ---
    private async Task<JwtIdentifier?> BuildIdentifierForServer(int serverIdx)
    {
        try
        {
            var playerIdentifier = await _dalamudUtil.GetPlayerNameHashedAsync().ConfigureAwait(false);
            if (string.IsNullOrEmpty(playerIdentifier)) return _lastJwtIdentifier;

            if (_serverManager.GetServerByIndex(serverIdx).UseOAuth2)
            {
                var (OAuthToken, UID) = _serverManager.GetOAuth2(out _, serverIdx)
                    ?? throw new InvalidOperationException("Requested OAuth2 but received null");

                return new JwtIdentifier(_serverManager.GetServerByIndex(serverIdx).ServerUri,
                    playerIdentifier, UID, OAuthToken);
            }
            else
            {
                var secretKey = _serverManager.GetSecretKey(out _, serverIdx)
                    ?? throw new InvalidOperationException("Requested SecretKey but received null");

                return new JwtIdentifier(_serverManager.GetServerByIndex(serverIdx).ServerUri,
                    playerIdentifier, string.Empty, secretKey);
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "BuildIdentifierForServer: Falling back to last identifier");
            return _lastJwtIdentifier;
        }
    }

    private async Task<string> GetNewTokenForServer(bool isRenewal, JwtIdentifier identifier, int serverIdx, CancellationToken ct)
    {
        Uri tokenUri;
        HttpResponseMessage result;

        var baseApi = _serverManager.GetServerByIndex(serverIdx).ServerUri
            .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
            .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase);
        ...
        if (!isRenewal)
        {
            if (_serverManager.GetServerByIndex(serverIdx).UseOAuth2)
            {
                tokenUri = NekoNet.API.Routes.MareAuth.AuthWithOauthFullPath(new Uri(baseApi));
                ...
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", identifier.SecretKeyOrOAuth);
                _logger.LogInformation("Sending OAuth Request (serverIdx={idx}) with auth {auth}", serverIdx, string.Join("", identifier.SecretKeyOrOAuth.Take(10)));
                result = await _httpClient.SendAsync(request, ct).ConfigureAwait(false);
            }
            else
            {
                tokenUri = NekoNet.API.Routes.MareAuth.AuthFullPath(new Uri(baseApi));
                var auth = identifier.SecretKeyOrOAuth.GetHash256();
                _logger.LogInformation("Sending SecretKey Request (serverIdx={idx}) with auth {auth}", serverIdx, string.Join("", identifier.SecretKeyOrOAuth.Take(10)));
                result = await _httpClient.PostAsync(tokenUri, new FormUrlEncodedContent([
                    new KeyValuePair<string, string>("auth", auth),
                    new KeyValuePair<string, string>("charaIdent", identifier.CharaHash),
                ]), ct).ConfigureAwait(false);
        ...
        {
            tokenUri = NekoNet.API.Routes.MareAuth.RenewTokenFullPath(new Uri(baseApi));
            HttpRequestMessage request = new(HttpMethod.Get, tokenUri.ToString());
            request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _tokenCache[identifier]);
            result = await _httpClient.SendAsync(request, ct).ConfigureAwait(false);
        }
    ...
    public async Task<string?> GetOrUpdateTokenForServer(int serverIdx, CancellationToken ct)
    {
        var jwtIdentifier = await BuildIdentifierForServer(serverIdx).ConfigureAwait(false);
        if (jwtIdentifier == null) return null;
        ...
        _logger.LogTrace("GetOrUpdateTokenForServer: Getting token (idx={idx})", serverIdx);
        return await GetNewTokenForServer(renewal, jwtIdentifier, serverIdx, ct).ConfigureAwait(false);
    }
